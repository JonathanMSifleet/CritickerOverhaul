const ROOT = 'https://fl6lwlunp9.execute-api.eu-west-2.amazonaws.com/dev';

export const DELETE_REVIEW = `${ROOT}/review/deleteReview`;
export const GET_FILM = `${ROOT}/getFilms`;
export const GET_FILM_BY_PARAM = `${ROOT}/getFilm`;
export const GET_PROFILE_BY_USERNAME = `${ROOT}/user/profile`;
export const GET_USER_AVATAR = `${ROOT}/user/avatar`;
export const GET_USER_RATING = `${ROOT}/review/getRating`;
export const LOGIN = `${ROOT}/login`;
export const IMPORT_RATINGS = `${ROOT}/review/importRatings`;
export const RATE_FILM = `${ROOT}/review/rateFilm`;
export const SIGNUP = `${ROOT}/signup`;
export const UPLOAD_USER_AVATAR = `${ROOT}/user/avatar`;
export const UPDATE_USER_PROFILE = `${ROOT}/user/profile`;
