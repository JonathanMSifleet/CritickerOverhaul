...
const importRatingsMySQL = async (event: { body: string }): Promise<IHTTP> => {
  ...
  const chunkedReviews = chunk(filteredReviews, 25) as any;

  try {
    const batchInserts = [] as Promise<BatchWriteItemCommandOutput | IHTTP>[];
    chunkedReviews.forEach(async (reviewChunk: any) => {
...
const batchInsertReviews = async (reviews: any): Promise<BatchWriteItemCommandOutput | IHTTP> => {
  const items = [] as { PutRequest: { Item: { [key: string]: AttributeValue } } }[];

  reviews.forEach((review: any) => items.push({ PutRequest: { Item: marshall(review) } }));
...
export const handler = middy(importRatingsMySQL).use(cors());
